  ...
  <div class="min-h-screen bg-gray-950 flex flex-col items-center p-4">
    <header class="w-full max-w-6xl mb-6">
      <div class="flex items-center justify-between mb-2">
        <h1 class="text-2xl font-bold text-green-400 text-center flex-1">棋类 AI 对弈与棋谱回放系统</h1>
        <button
          v-if="store.currentView !== 'lobby'"
          @click="store.switchToLobby()"
          class="px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-lg transition-colors text-sm"
        >
          ← 返回大厅
        </button>
      </div>
    ...
    <!-- Lobby View -->
    <div v-if="store.currentView === 'lobby'" class="w-full max-w-6xl">
      <Lobby />
    </div>

    <!-- Room View -->
    <div v-else-if="store.currentView === 'room'" class="w-full max-w-6xl">
      <div v-if="store.currentRoom?.status !== 'PLAYING'" class="mb-6">
        <RoomView />
      </div>

      <div v-if="store.currentRoom?.status === 'PLAYING' || store.status === 'playing'" class="flex flex-col lg:flex-row gap-6 w-full items-start justify-center">
        <!-- Board -->
        ...
        <div class="w-full lg:w-80 space-y-4">
          ...
          <div class="bg-gray-900 rounded-xl p-4 border border-gray-700">
            ...
            <div class="space-y-2 text-sm">
              ...
              <div v-if="store.myColor" class="flex justify-between">
                <span class="text-gray-400">你执</span>
                <span class="flex items-center gap-1">
                  <span class="inline-block w-3 h-3 rounded-full" :class="store.myColor === 1 ? 'bg-gray-800 border border-gray-600' : 'bg-white'"></span>
                  {{ store.myColor === 1 ? '黑棋' : '白棋' }}
                </span>
              </div>
          ...
          <div v-if="store.currentView === 'solo'" class="bg-gray-900 rounded-xl p-4 border border-gray-700">
          ...
          <!-- Room Players (only in room mode and playing) -->
          <div v-if="store.currentView === 'room' && store.currentRoom" class="bg-gray-900 rounded-xl p-4 border border-gray-700">
            <h3 class="text-lg font-bold text-green-400 mb-3">房间信息</h3>
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-gray-400">房间</span>
                <span class="text-white">{{ store.currentRoom.name }}</span>
              </div>
              <div class="space-y-2 mt-3">
                <div
                  ...
                  :class="player.id === store.currentUser.id ? 'bg-gray-800' : ''"
                >
                  <div class="flex items-center gap-2">
                    <span
                      class="inline-block w-3 h-3 rounded-full"
                      :class="player.color === 1 ? 'bg-gray-800 border border-gray-600' : 'bg-white'"
                    ></span>
                    <span class="text-white">{{ player.name }}</span>
                    <span
                      v-if="player.id === store.currentRoom.ownerId"
                      class="text-xs px-1.5 py-0.5 bg-yellow-600/30 text-yellow-400 rounded"
                    >房主</span>
                  </div>
                  <span
                    :class="player.isReady ? 'text-green-400' : 'text-gray-500'"
    ...
    <!-- Solo View -->
    <div v-else-if="store.currentView === 'solo'" class="w-full max-w-6xl">
